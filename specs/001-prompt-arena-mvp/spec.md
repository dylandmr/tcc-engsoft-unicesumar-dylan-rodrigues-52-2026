...
6. **Given** a completed comparison with at least two successful answers, **When** the user picks a
   judge model and requests the comparative analysis, **Then** an on-screen "key differences"
   analysis of the answers' content streams in below the telemetry summary — generated with the
   answers anonymized under neutral labels in randomized order and declaring no winner — and
   reopening the comparison from history shows the same recorded analysis without generating it
   again.
...
- **FR-021**: For a completed comparison with at least two successful answers, the system MUST let
  the user request a comparative analysis of the answers' content — the key differences in
  coverage, factual claims, structure, style, and concision — generated by a judge model the user
  picks per request from a provider's live model list (FR-020 applies: no default judge). The
  judge MUST NOT be one of the comparison's competing providers (a competitor judging its own race
  reintroduces self-preference bias); since at most four of the five providers compete, at least
  one non-competitor always exists. To mitigate known LLM-as-judge biases, the judge MUST receive
  the answers anonymized under neutral labels ("Modelo A", "Modelo B", …) in randomized order, and
  MUST be instructed to declare no winner and assign no scores. The generated analysis, the judge identity (provider and model), and
...
- **Comparison**: A single submitted prompt and the set of providers it was sent to. Key attributes:
  ...
  each selected provider (FR-020), a timestamp, and — once the user requests it — the recorded
  comparative analysis (text, judge provider/model, label→provider mapping — FR-021). Has one
  Provider Result per selected provider.

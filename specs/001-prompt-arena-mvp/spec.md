...
- **FR-021**: For a completed comparison with at least two successful answers, the system MUST let
  ...
  picks per request from a provider's live model list (FR-020 applies: no default judge). To
  mitigate known LLM-as-judge biases, the judge MUST receive the answers anonymized under neutral
  labels ("Modelo A", "Modelo B", …) in randomized order, and MUST be instructed to declare no
  winner and assign no scores. The generated analysis, the judge identity (provider and model), and
